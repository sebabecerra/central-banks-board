...
def main() -> None:
    ...
    for index, bank in enumerate(banks):
        ...
        print(f"[{started_at}] Iteration {index + 1}/{len(banks)}: {country} -> {central_bank_name}", flush=True)
        ...
        try:
            ...
            print(f"  Rows: {len(rows)} | Elapsed: {elapsed:.1f}s", flush=True)
            all_rows.extend(rows)
        except Exception as exc:
            elapsed = time.monotonic() - iteration_started
            print(f"  Error: {exc} | Elapsed: {elapsed:.1f}s", flush=True)
            raw_text = ""
        ...
        if index < len(banks) - 1 and args.delay_seconds > 0:
            print(f"  Sleeping {args.delay_seconds:.0f}s before next country...", flush=True)
